 ...
 * Email: hollingx@oregonstate.edu
 */
...
{
    struct listQueue *new = (struct listQueue *)malloc(sizeof(struct listQueue));
    new -> firstLink = new -> lastLink = NULL;
    return new;
...
{
    struct link *newLink = q -> firstLink -> next;
...
{
    s -> structSize = 0;
    s -> Q1 = malloc(sizeof(struct listQueue));
    s -> Q2 = malloc(sizeof(struct listQueue));
